...
БД находится в 3-ей нормальной форме, так как для нее очевидно выполняются следющие свойства. 

`1 НФ` – так как в каждой таблице для каждого атрибута содержится только одно значение.

`2 НФ` – все атрибуты зависят от первичного ключа целиком, а не от какой-то его части. 
...
Таблица `employee_version` является версионной типа `SCD2`.  В ней под каждую версию 
